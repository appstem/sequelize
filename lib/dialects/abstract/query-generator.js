...
var QueryGenerator = {
  ...
  */
  ...
  selectQuery: function(tableName, options, model) {
    ...
    // Force add WHERE to main query
    if (options.hasOwnProperty('outerWhere')) {
        console.dir(options.outerWhere);
      options.outerWhere = this.getWhereConditions(options.outerWhere, mainTableAs || tableName, model, options);
        console.dir(options.outerWhere);
      if (options.outerWhere) {
        mainQueryItems.push(' WHERE ' + options.outerWhere);
        // Walk the main query to update all selects
        _.each(mainQueryItems, function(value, key) {
          if(value.match(/^SELECT/)) {
            mainQueryItems[key] = this.selectFromTableFragment(options, model, mainAttributes, table, mainTableAs, options.outerWhere);
          }
        }.bind(this));
      }
    }
