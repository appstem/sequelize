...
var Transaction = module.exports = function(sequelize, options) {
  ...
  // assign different uuid generator and savepoint separator if using mssql
  if (sequelize.options.dialect === 'mssql') {
    getUuid = uuid24;
    savePointSeparator = '-sp-'
  } else {
    getUuid = uuid.v4;
    savePointSeparator = '-savepoint-';
  }

  if (this.parent) {
    this.id = this.parent.id;
    this.parent.savepoints.push(this);
    this.name = this.id + savePointSeparator + this.parent.savepoints.length;
  } else {
    this.id = this.name = getUuid();
  }
